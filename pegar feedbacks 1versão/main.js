...
function TorcerPraDarCerto() {
  async function handleFeedbacks() {
    //"inicia" o driver
    let driver = await new Builder().forBrowser(Browser.CHROME).build();

    //site de onde pego os dados 
    await driver.get(`https://www.glassdoor.com.br/Avalia%C3%A7%C3%B5es/Simpress-Avalia%C3%A7%C3%B5es-E222101_P52.htm?filter.iso3Language=por`)
...
function TransformToXLSX(data) {
  ...
  XLSX.writeFile(wb, 'Lista Feedback Maxwell Final.csv', { bookType: 'csv', type: 'binary' });
}
